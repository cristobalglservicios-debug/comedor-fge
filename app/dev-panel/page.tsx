...
import { Terminal, ShieldAlert, Users, Database, Activity, Power, Trash2, LogOut, Search, Save, AlertTriangle, CheckCircle2, Loader2, RefreshCw } from 'lucide-react';
...
export default function DevPanelPage() {
  ...
  const validarAccesoDev = async () => {
    ...
    // SEGURIDAD: Solo entra si tiene rol 'dev' o 'admin'
    if (miPerfil?.rol === 'dev' || miPerfil?.rol === 'admin') {
  ...
  const manejarBusqueda = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setBusqueda(valor);
    const filtrados = perfiles.filter(p => p.nombre_completo.toLowerCase().includes(valor) || p.dependencia.toLowerCase().includes(valor));
  ...
  const actualizarRol = async (id: string, nuevoRol: string) => {
    ...
    if (error) {
      ...
      setPerfilesFiltrados(prev => prev.map(p => p.id === id ? { ...p, rol: nuevoRol } : p));
    }
  ...
  const toggleSwitch = async (id: string, valorActual: boolean) => {
    ...
    if (error) {
      ...
      setConfiguraciones(prev => prev.map(c => c.id === id ? { ...c, valor: nuevoValor } : c));
    }
    setCargandoAccion(false);
  };
  ...
  const purgarTabla = async (tabla: string) => {
    ...
    setCargandoAccion(true);
    // En Supabase, para vaciar desde cliente sin clave de servicio, solemos borrar registros que no sean nulos.
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] font-mono text-slate-300 pb-10">
      ...
      <nav className="bg-slate-900 border-b border-slate-800 p-4 sticky top-0 z-50 shadow-2xl">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => router.push('/dashboard')} className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 px-4 py-2 rounded-lg text-xs font-bold transition-all">
      ...
      {mensaje && (
        <div className={`fixed top-20 right-4 z-50 p-4 rounded-xl shadow-2xl border flex items-center gap-3 animate-fade-in ${mensaje.tipo === 'exito' ? 'bg-emerald-950/90 border-emerald-500/50 text-emerald-400' : 'bg-red-950/90 border-red-500/50 text-red-400'}`}>
        ...
        {activeTab === 'roles' && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl animate-fade-in">
            <div className="p-6 border-b border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-4">
              <div>
                <h2 className="text-lg font-black text-white uppercase tracking-widest">Asignación de Roles</h2>
                <p className="text-xs text-slate-500">Cambia los permisos de acceso al sistema en tiempo real.</p>
              </div>
              <div className="relative w-full sm:w-72">
                <input type="text" value={busqueda} onChange={manejarBusqueda} placeholder="Buscar empleado..." className="w-full bg-slate-950 border border-slate-700 p-3 pl-10 rounded-xl text-xs text-white focus:border-emerald-500 outline-none transition-colors" />
            ...
            <div className="overflow-x-auto">
                ...
                <tbody className="divide-y divide-slate-800/50">
                  {perfilesFiltrados.slice(0, 50).map(p => (
                    <tr key={p.id} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="px-6 py-4 text-slate-400">{p.dependencia}</td>
                      <td className="px-6 py-4">
              ...
              {perfilesFiltrados.length === 0 && <div className="p-8 text-center text-slate-500 text-xs uppercase">No se encontraron usuarios</div>}
            </div>
          </div>
        )}
        ...
        {activeTab === 'switches' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
            ...
            {configuraciones.length === 0 && (
              <div className="col-span-full bg-slate-900 p-8 rounded-2xl border border-slate-800 text-center">
                <p className="text-slate-500 text-xs uppercase mb-4">No hay switches configurados. Corre el script SQL primero.</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl animate-fade-in">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-slate-500">Últimos 50 movimientos registrados en la tabla historial_comedor.</p>
              </div>
              ...
              <table className="w-full text-left text-[10px] whitespace-nowrap font-mono">
                <thead className="bg-slate-950/50 text-slate-500 uppercase tracking-widest border-b border-slate-800">
                  <tr>
                    ...
                    <th className="px-6 py-4">Empleado / Entidad</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-800/30 text-slate-400">
                  {logs.map((log, i) => (
                    <tr key={i} className="hover:bg-slate-800/30">
                      <td className="px-6 py-3 text-emerald-500">{log.id || 'N/A'}</td>
        ...
        {activeTab === 'danger' && (
          <div className="space-y-6 animate-fade-in">
            <div className="bg-red-950/20 border border-red-900/50 p-8 rounded-2xl shadow-2xl relative overflow-hidden">
              ...
              <p className="text-red-400/70 text-xs mb-8 max-w-2xl leading-relaxed">Las siguientes acciones ejecutan comandos DELETE directos. No hay papelera de reciclaje. Se requiere confirmación manual para evitar catástrofes.</p>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
                <div className="bg-slate-950/50 border border-red-900/30 p-6 rounded-xl">
                  <h4 className="text-white font-bold text-sm uppercase mb-1">Vaciar Vales Quemados</h4>
                  <p className="text-slate-500 text-[10px] mb-4">Elimina el registro de folios QR. (Se hace si los IDs se empiezan a repetir, muy raro).</p>
                  <button onClick={() => purgarTabla('vales_quemados')} className="bg-red-600/20 hover:bg-red-600/40 text-red-500 border border-red-500/50 px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all w-full">Purgar vales_quemados</button>
                </div>

                <div className="bg-slate-950/50 border border-red-900/30 p-6 rounded-xl">
                  <h4 className="text-white font-bold text-sm uppercase mb-1">Vaciar Reservas de Menú</h4>
                  <p className="text-slate-500 text-[10px] mb-4">Elimina todos los apartados actuales. (Si se hace una prueba fallida masiva).</p>
                  <button onClick={() => purgarTabla('reservas_comedor')} className="bg-red-600/20 hover:bg-red-600/40 text-red-500 border border-red-500/50 px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all w-full">Purgar reservas_comedor</button>
                </div>
              </div>
            </div>

            <div className="bg-amber-950/20 border border-amber-900/50 p-8 rounded-2xl shadow-2xl relative overflow-hidden">
               <h2 className="text-xl font-black text-amber-500 uppercase tracking-widest mb-2 flex items-center gap-2"><Database size={24}/> Operaciones Masivas (Cron Jobs manuales)</h2>
               <p className="text-amber-400/70 text-xs mb-6 max-w-2xl leading-relaxed">Ejecuta rutinas globales sobre la tabla de perfiles.</p>
               
               <div className="bg-slate-950/50 border border-amber-900/30 p-6 rounded-xl w-full md:w-1/2">
                  <h4 className="text-white font-bold text-sm uppercase mb-1">Reinicio Semanal de Saldos</h4>
                  <p className="text-slate-500 text-[10px] mb-4">Devuelve los vales a su estado original para la nueva semana a todos los empleados.</p>
                  <button onClick={reiniciarTicketsSemanales} className="bg-amber-600/20 hover:bg-amber-600/40 text-amber-500 border border-amber-500/50 px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all w-full">Ejecutar Reinicio Global</button>
               </div>
            </div>
          </div>
        )}
...
import { crearUsuarioGlobal } from '../admin/actions'; // Asegúrate de que la ruta coincida con tu archivo de acciones
